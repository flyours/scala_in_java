package com.jeff.scala.drills.circuit.breaker
...
class FastSlowAkkademyDb extends Actor {
    ...
    override def receive = {
        case SetRequest(key, value) =>
            log.error("received SetRequest - key: {} value: {}", key, value)
            map put (key, value)
    ...
    def respondToGet(key: String): Unit = {
        map.get(key) match {
            ...
            case None => sender() ! Status.Failure(new RuntimeException(key))
        }
    }
}
