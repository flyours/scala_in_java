...
object StateMonad {
    val logger = LoggerFactory.getLogger(StateMonad.getClass)
    ...
    trait State[S, +A] {
        ...
        def flatMap[B](f: A => State[S, B]): State[S, B] = state(apply(_) match {
            case (s, a) =>
                logger.debug("a={}", a)
                logger.debug("s={}", s)
                logger.debug("f={}", f.getClass)
                logger.debug("f(a)={}", f(a))
                logger.debug("f(a)(s)={}", f(a)(s))
                f(a)(s)
        })
    }
...
object PriceCalculator {
    ...
    def test() = {
        def modifyPriceState(f: PriceState => Double) = modify[PriceState](s => s.copy(price = f(s)))
        val basePrice = modifyPriceState(findBasePrice)
        val initialPriceState = PriceState("productId", "stateCode", 0.0)
        val res = basePrice.apply(initialPriceState)
        println(res)
    }

    def main(args: Array[String]): Unit = {
        //
        //        calculatePriceWithLog("product name","state")
        //
        val res = calculatePrice2("jeff", "beijing")
        println(res)
